...
# run TbusOS.
#
# Copyright (C) 2023.01.02 by liaowenxiong <571550728@qq.com>
#
...
do_run()
{
	...
	if [ "$1" = $sd_arg ]
	then
		QEMU_DEVICE_ARG="-drive if=sd,format=raw,file=./rootfs.ext4,id=sd0"
		CMDLINE=${CMDLINE_SD}
	elif [ "$1" = $virt_arg ]
	then
		QEMU_DEVICE_ARG="-device virtio-blk-device,drive=hd0 -drive file=./rootfs.ext4,format=raw,id=hd0"
